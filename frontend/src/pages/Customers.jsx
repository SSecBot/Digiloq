...
const emptyForm = {
  ...
  day_of_month: 1, account_id: "none", description: "", active: true,
};
...
export default function Customers() {
  ...
  const remove = async (id) => {
    if (!window.confirm("Müşteri ve gelecekteki ödenmemiş alacakları silinsin mi? Ödenmiş geçmiş korunur.")) return;
    try {
  ...
  const startEdit = (c) => {
    ...
    setForm({
      ...
      day_of_month: c.day_of_month, account_id: c.account_id || "none",
  ...
  return (
    <div className="space-y-6" data-testid="customers-page">
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setForm(emptyForm); setEditing(null); } }}>
          ...
          <DialogContent className="bg-card border-border max-w-lg">
            ...
            <DialogFooter>
              ...
              <Button data-testid="submit-customer-btn" onClick={submit} className="bg-brand text-white hover:bg-brand/90">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {items.map((c) => (
          <div key={c.id} className="bg-card border border-border rounded-md p-5 hover:border-zinc-700 transition-colors" data-testid={`customer-card-${c.id}`}>
            <div className="flex items-start justify-between">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button data-testid={`customer-menu-${c.id}`} className="text-muted-foreground hover:text-foreground p-1">
                ...
                <DropdownMenuContent align="end" className="bg-card border-border">
                  ...
                  <DropdownMenuItem onClick={() => remove(c.id)} className="text-expense focus:text-expense">
            ...
            <div className="mt-5 flex items-baseline justify-between">
              ...
              <div className="text-right">
                <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Vade Günü</div>
                <div className="font-mono text-lg mt-1">{c.day_of_month}.</div>
              </div>
            </div>
